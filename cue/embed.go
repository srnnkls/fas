...
//
// The stdlib is organized as a set of sub-packages under cue/ — catalog,
// hook, tool, command, path, escalation, action, flag — each its own CUE
// package at its own import path. StdlibFS re-embeds the whole tree so the
// rule loader can stage a matching `cue.mod/pkg/...` overlay and resolve
// per-sub-package imports.
package cue
...
//go:embed schema.cue catalog/*.cue hook/*.cue tool/*.cue command/*.cue path/*.cue escalation/*.cue action/*.cue flag/*.cue
...
// hook/, tool/, command/, path/, escalation/, action/, flag/). Callers mount
// it into a `cue/load` overlay so rule files can resolve
...
// hook, tool, command, path, escalation, action, or flag sub-package.
